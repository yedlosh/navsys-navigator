...
const animations = {
  ...
  navigationHR: function (addressing, params) {
    ...
    return function (pixelData) {
      ...
      if(!alert || alertCount < 40) {
        for (let i = 0; i < addressing.length; i++) {
          ...
          pixelData[addressing[i]] = pattern[(i - step) % 6];
        }
      } else {
        for (let i = 0; i < addressing.length; i++) {
  ...
  navigation: function (addressing, params) {
    ...
    return function (pixelData) {
      ...
      if (++skip > 8) {
        ...
        if(!alert || alertCount < 40) {
          for (let i = 0; i < addressing.length; i++) {
            buffer[i] = pattern[(i - step) % 5];
          }
        } else {
          for (let i = 0; i < addressing.length; i++) {
            buffer[i] = constants.ALERT_WARNING_COLOR;
          }
        }
      }
      for (let i = 0; i < addressing.length; i++) {
        pixelData[addressing[i]] = buffer[i];
      }
    }
  },
  singleSnake: function (addressing, params) {
    const color = parseIntParam(params.color);
    const alert = parseBoolParam(params.alert);
    let step = -1;
    let skip = -1;
    ...
    return function (pixelData) {
      ...
      if (++skip > 2) {
        ...
        if(!alert || alertCount < 40) {
          ...
          if (++lastIndex > addressing.length - 1) {
            lastIndex = 0;
          }

          buffer[lastIndex] = color;
        } else {
          for (let i = 0; i < addressing.length; i++) {
            buffer[i] = constants.ALERT_WARNING_COLOR;
          }
      ...
      for (let i = 0; i < addressing.length; i++) {
        pixelData[addressing[i]] = buffer[i];
        buffer[i] = shadeColor(buffer[i], -0.05);
      }

    }
  },
  multiSnake: function (addressing, params) {
    const color = parseIntParam(params.color);
    const alert = parseBoolParam(params.alert);

    const stepCount = config.ANIMATION_STEPS;
    const skipCount = config.ANIMATION_SKIP;
    const fadeAmount = config.ANIMATION_FADE;
    ...
    return function (pixelData) {
      ...
      if (++skip >= skipCount) {
        ...
        if(!alert || alertCount < 40) {
          for (let i = 0; i < addressing.length; i++) {
            buffer[i] = shadeColor(buffer[i], -fadeAmount);
            if((i - step) % stepCount === 0)
              buffer[i] = color;
          }
        } else {
          for (let i = 0; i < addressing.length; i++) {
  ...
  multiSnakeFadeIn: function (addressing, params) {
    const color = parseIntParam(params.color);
    const alert = parseBoolParam(params.alert);
    ...
    return function (pixelData) {
      ...
      if(!alert || alertCount < 40) {
        for (let i = 0; i < addressing.length; i++) {
          if((i - step) % stepCount === 0) {
            buffer[i] = blendColors(buffer[i], color, 1 / (skipCount / 1.66));
          }
        }
      } else {
        for (let i = 0; i < addressing.length; i++) {
  ...
  strobo: function (addressing, params) {
    const color = parseIntParam(params.color);
    const steps = parseIntParam(params.steps);
    let step = -1;
    return function (pixelData) {
      if (++step > steps) {
        step = 0;
