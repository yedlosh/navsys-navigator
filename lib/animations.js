...
const animations = {
  ...
  navigation: function (addressing, params) {
    const color = getIntParam(params, 'color');
    let step = -1;
    return function (pixelData) {
      if (++step > 4) {
        step = 0;
      ...
      for (let i = 0; i < addressing.length; i++) {
        pixelData[addressing[i]] = shadeColor(color, - 1 / (0.01 + ((i - step) & 5)));
      }
    }
  },
...
function getIntParam(params, paramName) {
  const param = params[paramName];
  let paramInt;
  try {
    paramInt = parseInt(param);
  } catch (err) {
    throw new Error("Navigation Animation: No or invalid color");
  }
  if(!paramInt) {
    throw new Error("Navigation Animation: No or invalid color");
  }
