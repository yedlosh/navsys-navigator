...
function startAnimator() {
  ...
  navInterval = setInterval(navSwitcher, 2000);
}
...
export function startAnimation(id, type, params, from, to) {
  ...
  if(type === 'navigation') {
    runningNavigations.push(animations.getAnimation(id, type, params, from, to));
    if(params.alert) {
      audio.play(constants.ALERT_AUDIO);
    }
  } else {
...
export function stopAnimation(id, clear) {
  ...
  if(animation) {
    ...
    const navigation = runningNavigations.find((animation) => animation.id === id);
    if(navigation) {
...
export function runner() {
  ...
  if(runningNavigations.length > 0){
    runningNavigations[runningNavigation]((animation) => animation.tick(pixelData));
  }
...
function navSwitcher() {
  if(runningNavigations.length > 0) {
    stopAnimation(runningNavigations[runningNavigation].id, true);
