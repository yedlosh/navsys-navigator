...
export function playCue(req, res, next) {
  let cue = req.params.id;
  const cueFiles = constants.AUDIO_CUE_FILES[cue];
  if (cueFiles) {
    if(cueFiles.length > 0) {
      const filename = cueFiles[Math.floor(Math.random()*cueFiles.length)];
      try {
        audio.play(filename);
        return res.status(200).json({success: "Playing cue", cue: cue});
      } catch (err) {
        return res.status(400).json({error: "Audio cue failed to start!", message: err});
      }
    } else {
      return res.status(500).json({error: "No audio files for this cue!"});
    }
  }
  else {
    return res.status(400).json({error: "Parameters missing or invalid!"});
  }
}
